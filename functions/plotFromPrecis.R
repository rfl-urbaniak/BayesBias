...
plotFromPrecis <- function (precis, dataset, ylims = c(.5,1.2)){
  results <- list()
  ...
  results$plotIndividual <- ggplot() + geom_point(data = DFindividual, aes( x = reorder(pw, desc(pw)), y = mean, color = type),
    ...
    geom_hline(yintercept = 1, lty = 2, size = .2, alpha = .3) +labs(title = "Protected-word-relative cosine distances", subtitle = "Means with 89% highest posterior density intervals")+ 
    theme(legend.position='none') + ylim(ylims)
  ...
  results$plotOverall  <- ggplot() + geom_point(data = DFoverall, aes( x = type, y = mean, color = type), size = 1) +
    ...
    scale_color_manual(values = c("associated" ="orangered4", 
                                  "different" = "chartreuse4",
                                  "human" = "skyblue",
                                  "none" = "grey"))+ theme(legend.position = c(0.9, 0.8))+labs(title = "Connection-relative distances", 
