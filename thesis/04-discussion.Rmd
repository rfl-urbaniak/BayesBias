...
In some cases the final cosine distance is indeed close to neutral words, but in other cases the change is minor. What is more, including uncertainty shows how in some cases it is not clear why a word is according to this method
classified as anomalous. The fact that uncertainty for `assosiated` and `different` class is so high leads to the situation where in the boundaries of an uncertainty of some attributes cosine distance, one can find the neutral words as well. It makes the usage and interpretation of this metric extremely complicated and unclear. 
...
// see the chapter 3 to write more 
